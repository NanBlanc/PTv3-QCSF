...
@TRANSFORMS.register_module()
class StrengthJitter(object):
    def __init__(self, sigma=0.01, clip=0.04):
        self.sigma = sigma
        self.clip = clip

    def __call__(self, data_dict):
        if "strength" in data_dict.keys() :
            values=data_dict["strength"]
            values += np.clip(self.sigma * np.random.randn(*values.shape), -1*self.clip, self.clip)
            data_dict["strength"]=values/np.max(values)
        return data_dict
